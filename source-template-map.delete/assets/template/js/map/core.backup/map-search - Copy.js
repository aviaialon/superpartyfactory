...
MapConfigs.mainContainerSelector = '.map-search';
// Location selector button
MapConfigs.locationSelector = '#gmapcurrentloc1146989938';
// Location input field (the one that has the autocomplete feature)
MapConfigs.locationInputFieldSelector = '#directory-search-location';
// Search button (from location form)
MapConfigs.searchButtonSelector = '.search-button';
// Search form selector
MapConfigs.formSelector = 'form';


!(function () {
	var SearchInForm = flight.component(function () {
        this.defaultAttrs({
            locationSelector: MapConfigs.locationInputFieldSelector,
            searchButtonSelector: MapConfigs.searchButtonSelector,
            formSelector: MapConfigs.formSelector
        });
        this.triggerLocationFieldsChanged = function (e) {
            ...
            this.trigger(MapConfigs.mainContainerSelector, "uiHeaderFieldsChanged", {
                location: location
            })
        };
        ...
        this.after("initialize", function () {
			...
			this.on("location.updated", function(event) {
				$(MapConfigs.locationInputFieldSelector).val(this.select("locationSelector").val());
    ...
    var Map = flight.component(function () {
        ...
        this.plotResults = function (e, data) {
			...
			if (!data.listingData.length) {
				console.log('--------- NO DATA.. RETURNING VOID! --------------');
                return
            }
...
!(function () {
    var SidebarHeader = flight.component(function () {
        ...
        this.updateAppliedFilters = function (e, data) {
			console.log(' ------------- updateAppliedFilters ------------------------------------');
...
!(function () {
    var FindMe = flight.component(function () {
        this.defaultAttrs({
            buttonSelector: MapConfigs.locationSelector,
            inputSelector: MapConfigs.locationInputFieldSelector
        });
		this.findLocationByAddress = function (event) {
			...
			objMapDirectionApi.getAddressGeocode($(event.target).val(), function(data, result) {
				...
				$('#lng').val(data.lng);
				target.val(data.fma);
				component.trigger(MapConfigs.mainContainerSelector, "uiHeaderFieldsChanged", {
					location: data.fma
				})
        ...
        this.after("initialize", function () {
			...
			$(MapConfigs.locationInputFieldSelector).on('keypress', function(e){
...
!(function () {
    ...
    var MapSearchPage = flight.component(function () {
        this.after("initialize", function () {
            ...
            Sidebar.attachTo("#directory-advanced-search");
