<?php
/**
 * Crype Administration Class
 *
 * This class controls the Crypting scope
 ...
 *
 */
class AesCrypt extends \Exception
{
    ...
    const AesCrypt_ENCRYPT_CIPHER_BITS_128 = 128;
    const AesCrypt_ENCRYPT_CIPHER_BITS_192 = 192;
    const AesCrypt_ENCRYPT_CIPHER_BITS_256 = 256;

    /**
     * Returns a class instance
     */
    public static final function getInstance($encryptionKey = null)
    {
        if (true === empty($encryptionKey)) {
            $Application   = \Core\Application::getInstance();
            $encryptionKey = $Application->getConfigs()->get('Application.Core.Crypt.AesCryptSalt');
        }

        $objCrypto   = new \Core\Crypt\AesCrypt();
        $objCrypto->setEncryptionKey($encryptionKey);
        $objCrypto->setEncryptionBits(AesCrypt::AesCrypt_ENCRYPT_CIPHER_BITS_128);
        return ($objCrypto);
    }
    ...
    public function mysqlEncrypt($strText = NULL)
    {
        ...
        if (false === function_exists('mcrypt_encrypt')) {
            ...
            $arrEncrypedData     = $objDatabase->query('SELECT AES_ENCRYPT("' . $objDatabase->escape($strText) . '", "' . constant('__ENCRYPTION_KEY__') . '") encodedData');
            $arrResponseData     = array_shift($arrEncrypedData);
            //$strEncryptedData     = (utf8_encode($arrResponseData['encodedData']));
            $strEncryptedData     = ($arrResponseData['encodedData']);
        } else {
    ...
    {
        $strDecryptedData = NULL;
        if (false === function_exists('mcrypt_encrypt'))
        {
            $objDatabase         = DATABASE::getInstance();
            $arrEncrypedData     = $objDatabase->query('SELECT AES_DECRYPT("' . $objDatabase->escape($strText) . '", "' . constant('__ENCRYPTION_KEY__') . '") decodedData');
            $arrResponseData     = array_shift($arrEncrypedData);
            $strDecryptedData     = $arrResponseData['decodedData'];
        }
        else
        {
            ...
            $strDecryptedData = rtrim($strText, "\0..\16");
        }
     ...
     * @param Integer - The encryption bits algorithim to use - Default: AesCrypt::AesCrypt_ENCRYPT_CIPHER_BITS_128
     * @return vois
     */
     public function setEncryptionBits($intBits = AesCrypt::AesCrypt_ENCRYPT_CIPHER_BITS_128)
     {
