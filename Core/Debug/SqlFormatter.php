 ...
 * Debugging class
 *
 * This class controls the SQL Formatting scope
 *
 * @namespace    Core
 * @package      Core
 * @subpackage   Debug
...
class SqlFormatter
{
     ...
     * @return string
     */
	public static final function getInstance($sqlQuery) {
		return \Core\Debug\SqlFormatter::format($sqlQuery);	
	}
    ...
    protected static function init()
    {
        ...
        usort(self::$reserved, array('\Core\Debug\SqlFormatter', 'sortLength'));

        // Set up regular expressions
        self::$regex_boundaries = '('.implode('|',array_map(array('\Core\Debug\SqlFormatter', 'quote_regex'),self::$boundaries)).')';
        self::$regex_reserved = '('.implode('|',array_map(array('\Core\Debug\SqlFormatter', 'quote_regex'),self::$reserved)).')';
        self::$regex_reserved_toplevel = str_replace(' ','\\s+','('.implode('|',array_map(array('\Core\Debug\SqlFormatter', 'quote_regex'),self::$reserved_toplevel)).')');
        self::$regex_reserved_newline = str_replace(' ','\\s+','('.implode('|',array_map(array('\Core\Debug\SqlFormatter', 'quote_regex'),self::$reserved_newline)).')');

        self::$regex_function = '('.implode('|',array_map(array('\Core\Debug\SqlFormatter', 'quote_regex'),self::$functions)).')';
