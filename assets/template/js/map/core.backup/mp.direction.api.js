 ...
 * @param 	object 		objOptions		- The field input options
 * @return 	void
 */
MAP_DIRECTION_API.prototype.setAutoCompleteFieldInput = function(strFieldInput, fnOnInputChangeCallback, objOptions)
{
	var me = this;
	var callBackMethod = fnOnInputChangeCallback;
	var objOptions = (typeof objOptions == "undefined" ? {} : objOptions);
	...
	if (typeof document.getElementById(strFieldInput) !== "undefined")  {
		...
		if (! this.getVariable('AutoCompleteFieldInputId')) {
			...
			this.setVariable('AutoCompleteFieldInput', 
				new google.maps.places.Autocomplete(document.getElementById(this.getVariable('AutoCompleteFieldInputId')), objOptions)
			);	
		}
	...
	objGeoCoder.geocode({'address': strAddress}, function(arrResults, status) {
		if (status == google.maps.GeocoderStatus.OK) {
			successCallbackHandler({
				...
				'fma':  arrResults[0].formatted_address
			}, arrResults);
		} else {
			errorCallbackHandler(status, strAddress);
		}
